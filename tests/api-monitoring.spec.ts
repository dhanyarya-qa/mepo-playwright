...
test.describe('API Response Monitoring @regression', () => {
  ...
  for (const path of pages) {
    ...
    test(`should have no console errors on ${path}`, async ({ page }) => {
      ...
      // Filter out common benign errors in dev environment
  ...
  test('should have page load time under 10 seconds', async ({ page }) => {
    const pages = ['/', '/about', '/products', '/activities', '/contact-us'];
    ...
    for (const path of pages) {
      ...
      // Increased timeout to 10 seconds for more realistic expectations
      expect(loadTime, `${path} loaded in ${loadTime}ms`).toBeLessThan(10000);
    }
  });

  test('should have all API responses within 10 seconds', async ({ page }) => {
